...
from files import import_csv, export_csv
#convention:
# title is string
...
class SearchFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        self.import_button = ttk.Button(self, text="Import From CSV File", command = self.import_data_wrap, style="TButton")
        self.export_button = ttk.Button(self, text="Export Data As CSV File", command=self.export_data_wrap, style="TButton")
    ...
    def create_widget(self):
        ...
        self.import_button.grid(row=11, column=0)
    ...
    def import_data_wrap(self):
        path = browse_for_file()
        try:
            import_csv(path)
        except UnSupportedFileFormat:
            messagebox.showerror(title='Fomat Error', message='Un Supported File Format,\n'
                                                              'Please Supply a valid csv file.  ' )

    def export_data_wrap(self):
        file_path = browse_for_directory()
        export_csv(show_all(),file_path)
